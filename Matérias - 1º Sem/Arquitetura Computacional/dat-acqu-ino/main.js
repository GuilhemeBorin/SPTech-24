...
const serial = async (
    ...
    arduino.pipe(new serialport.ReadlineParser({ delimiter: '\r\n' })).on('data', async (data) => { 
        ...
        const dht11Umidade = parseFloat(valores[0]);                           //POSSO APAGAR OS SENSORES INUTILIZADOS
        const dht11Temperatura = parseFloat(valores[1]);
        const lm35Temperatura = parseFloat(valores[2]);
        const luminosidade = parseFloat(valores[3]);
        const chave = parseInt(valores[4]); // sensor de proximidade

        // Armazena os valores dos sensores nos arrays correspondentes
        valoresDht11Umidade.push(dht11Umidade);                                //POSSO APAGAR OS SENSORES INUTILIZADOS
        ...
        if (HABILITAR_OPERACAO_INSERIR) {
            ...
            await poolBancoDados.execute(
                ...
                [dht11Umidade, dht11Temperatura, luminosidade, lm35Temperatura, chave] // TROCAR O "?" PELOS VALORES EM AZUL AQUI <<
            );
